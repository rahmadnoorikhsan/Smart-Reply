...
class ChatViewModel : ViewModel() {
    ...
    private fun initSmartReplyOptionsGenerator() {
        ...
        _smartReplyOptions.addSource(chatHistory) { convertsations ->
            val isPretendingAsAnotherUser = pretendingAsAnotherUser.value

            if (isPretendingAsAnotherUser != null && convertsations.isNullOrEmpty()) {
                return@addSource
            } else {
                generateSmartReplyOptions(convertsations, isPretendingAsAnotherUser!!)
                    .addOnSuccessListener { result ->
                        _smartReplyOptions.value = result
                    }
            }
